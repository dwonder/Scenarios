...
const ModuleCompleteScreen: React.FC<ModuleCompleteScreenProps> = ({ onPlayAgain, onRetry, points, bonus, isGameComplete, onShowLeaderboard, passed }) => {
    ...
    if (!passed && !isGameComplete) {
        return (
            <div className="text-center flex flex-col items-center justify-center animate-fade-in" style={{height: '70vh'}}>
                <h2 className={`text-4xl sm:text-5xl font-extrabold text-amber-400 tracking-tight`}>
                    Almost There!
                </h2>
                <p className="mt-4 max-w-2xl text-lg text-slate-400">
                    You didn't quite meet the passing score of 70 for this module. Repetition is key to mastery!
                </p>

                <div className="mt-6 text-2xl font-semibold text-slate-200">
                    Your Score: <span className="text-amber-400">{points}</span>
                </div>

                <div className="mt-8 flex flex-col sm:flex-row gap-4">
                    <button
                        onClick={onRetry}
                        className="px-8 py-3 bg-[#D40511] text-white font-bold rounded-full hover:bg-[#a7040e] transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-4 focus:ring-[#D40511]/50 shadow-lg shadow-[#D40511]/30"
                    >
                        Try Again
                    </button>
                    <button
                        onClick={onPlayAgain}
                        className="px-8 py-3 bg-slate-700 text-white font-bold rounded-full hover:bg-slate-600 transition-all"
                    >
